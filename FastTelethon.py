...
class UploadSender:
    ...
    def __init__(
        ...
        if big:
            ...
            self.request = SaveFilePartRequest(
                file_id, index, b""
            )
        self.stride = stride
        self.previous = None
...
class ParallelTransferrer:
    ...
    async def _create_sender(self) -> MTProtoSender:
        ...
        if not self.auth_key:
            ...
            auth = await self.client(
                ExportAuthorizationRequest(self.dc_id)
            )
    ...
    async def init_upload(
        ...
        connection_count = (
                connection_count or
                self._get_connection_count(file_size)
        )
...
async def upload_file(
    ...
    res = (
        await _internal_transfer_to_telegram(
            client, file, file_name, progress_callback
        )
    )[0]
